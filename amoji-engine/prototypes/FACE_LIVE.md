...
- **Prefs**: Face Live restores from `localStorage`; **Export prefs** / **Import prefs** JSON; **Copy share link**; **Show QR**; **Clear deep-link**; **share audit** with filter / search / **Export audit** / **Clear audit**
- **TTS speak**: Mock / HTTP / gateway presets + **Probe gateway** (SLA sparkline → probe detail toast → **Copy probe**)
- **Compound emotions**: lifecycle cancelled by emotion / emblem / finger / chassis / pack / **Live Link remap** picks
