...
- **Disney Extreme**: optional cartoon deform tier — master toggle (`X`) · body apply (`B`) + shape × (`[` `]`) · body × (`-` `=`, enables Apply to body/head) · eyes × (`,` `.`) · mouth × (`;` `'`) · Copy (`C`) · Reset (`R`) · Help (`H` / `?`) · Esc clears sticky status flash (persisted, live value labels, Reset × defaults, Copy summary, shared hotkey help legend, sticky status flash hold ~2.2s, prefs summary tooltip + landing toast); stronger Extreme-band ease/recipe overdrive gains; `computeDisneyExtremeIntensities` for shape/body/residual ints; `amplifyDisneyExtremeMorphs` amplifies eye/brow/lid + mouth/lip/jaw morphs late in the frame (cap 2.0); extrapolates Layer B + scales head/neck bones; live `formatDisneyExtremeLiveHud` status + X pill (`shapeInt`) and body status / HUD show `bodyInt` when Extreme is on
