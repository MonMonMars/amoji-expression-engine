...
- **Disney Extreme**: optional cartoon deform tier — master toggle + shape × · body/head × · eyes × · mouth × (persisted, live value labels); `amplifyDisneyExtremeMorphs` amplifies eye/brow/lid + mouth/lip/jaw morphs late in the frame (cap 2.0); extrapolates Layer B + scales head/neck bones; body status / HUD show `bodyInt` when Extreme is on
