...
describe('Phase 493 Extreme label a11y docs sync', () => {
  it('syncs readme/docs notes and preserves Extreme button count', () => {
    const readme = readFileSync(join(root, 'README.md'), 'utf8');
    expect(readme).toContain('| Phase 493 | Extreme label a11y docs sync | Done |');
    const md = readFileSync(join(root, 'prototypes/FACE_LIVE.md'), 'utf8');
    expect(md).toContain('spark label keyboard');
    expect(md).toContain('filter input aria');
    expect(md).toContain('status ⇧Enter copy');
